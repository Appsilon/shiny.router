...
how you can use `shiny.router` in a Rhino application.
...
To speed things up, we will use a basic version of the Rhino application that
you can build by following the [Rhino tutorial](https://appsilon.github.io/rhino/articles/tutorial/create-your-first-rhino-app.html).
...
Next, we need to fill the application with content.
...
Now, let us move the table and the chart to separate pages using `shiny.router`.
...
Using only the URL to change the view is not the best UX,
...
- an introduction with buttons that will move a user to either chart or a table.
...
Now, we will implement the opposite feature - we will add a page size dropdown to the table
